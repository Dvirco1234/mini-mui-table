      ...
      {data.length === 0 ? (
        ...
        data.map((photo) => (
          ...
          >
            ...
            <div className="p-3">
              <h3 className="text-sm font-medium line-clamp-2 h-10" title={photo.title}>
                {photo.title}
              </h3>
...
export function PhotoGalleryTable() {
  ...
  useEffect(() => {
    const fetchPhotos = async () => {
      ...
      try {
        ...
        const albums = [...new Set(data.map(photo => photo.albumId))].sort((a, b) => a - b);
  ...
  useEffect(() => {
    ...
    // Apply album filter
    if (albumFilter) {
      result = result.filter(photo => photo.albumId.toString() === albumFilter);
    }
